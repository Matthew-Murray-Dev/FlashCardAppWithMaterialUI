...
function View() {
  ...
  return (
    <div>
      {<BreadcrumbNav deck={deck} />}
      <Switch>
        <Route exact path={url}>
          <h4>{deck.name}</h4>
          {deck.description}

          <div className="row pl-3 mt-2 justify-content-between">
        ...
        <Route path={`${url}/edit`}>
          {deck.name && <EditDeck deck={deck} />}
        </Route>
        <Route exact path={`${url}/cards/:cardId/edit`}>
          <CardFormHolder deck={deck} />
        </Route>
